...
export function ShoppingCartProvider({ children }: ShoppingCartProviderProps) {
  ...
  const getListDetailCart = async () => {
    try {
      ...
      if (res.status) {
        // Xử lý dữ liệu để đảm bảo item.image là URL đơn
        const processedProducts = res?.data.map((item: IDetailProductCart) => ({
          ...item,
          image: item.image.includes(",") ? item.image.split(",")[0] : item.image, // Lấy ảnh đầu tiên nếu là chuỗi
        }));
        setListProducts(processedProducts || []);
      }
    } catch (error) {
      console.log(error);
    }
